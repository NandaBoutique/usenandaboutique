...
async function main() { console.log("Iniciando verificações no navegador...");
  ...
  await check("cliente recebe saudação, sessão restaurada e cadastro solicita confirmação", async () => {
    ...
    assert.equal(await evaluate("document.querySelector('#adminToggle').hidden"), true); assert.equal(await evaluate("document.querySelector('#stockAlertsToggle').hidden"), true);
  ...
  await check("sino isolado lista reposições, e o painel salva quantidades e guia da peça", async () => {
    await fresh(); await login();
    assert.equal(await evaluate("document.querySelector('#stockAlertsToggle').hidden"), false); assert.equal(await evaluate("document.querySelector('#stockAlertsBadge').textContent"), '3');
    assert.equal(await evaluate("document.querySelectorAll('#stockAlerts').length"), 0);
    await click('#stockAlertsToggle'); await until("document.querySelector('#stockAlertsModal').open", 'modal de alertas');
    const stockAlerts = await browser(() => [...document.querySelectorAll('#stockAlertsList .stock-alert-item')].map(item => item.textContent.replace(/\s+/g, ' ').trim()));
    assert.equal(stockAlerts.length, 3); assert(stockAlerts.some(text => /Vestido Aurora.*Tamanho: M.*2 unidades/i.test(text))); assert(stockAlerts.some(text => /Vestido Aurora.*Tamanho: G.*0 unidades/i.test(text))); assert(stockAlerts.some(text => /Saia Coração.*Tamanho: M.*0 unidades/i.test(text)));
    await key('Escape', 'Escape', 27); await until("!document.querySelector('#stockAlertsModal').open && document.activeElement.id === 'stockAlertsToggle'", 'fechamento e foco do sino'); await click('#adminToggle');
    ...
    assert.equal(await evaluate("document.querySelector('#stockAlertsBadge').textContent"), '4');
  ...
  await check("controles e diálogos acessíveis em 320, 390, 768, 1024, 1440 e 1920 px", async () => {
    ...
    for (const width of widths) {
      ...
      const quickActions = await browser(() => { const header = document.querySelector('.store-header').getBoundingClientRect(); return [...document.querySelectorAll('#adminQuickActions button')].map(button => { const rect = button.getBoundingClientRect(); return { width: rect.width, height: rect.height, top: rect.top, right: rect.right, headerBottom: header.bottom, viewportWidth: innerWidth }; }); });
      assert(quickActions.every(action => action.width >= 43.9 && action.height >= 43.9 && action.top >= action.headerBottom && action.right <= action.viewportWidth + 1), 'atalhos administrativos ' + width);
      await click('#adminToggle');
